...
extern "C" JNIEXPORT void JNICALL
Java_com_melihhakanpektas_flutter_1midi_1pro_FlutterMidiProPlugin_setGlobalTuning(JNIEnv* env, jclass clazz, jint sfId, jdouble cents) {
    if (synths.find(sfId) == synths.end()) return;
    // Pitch bend range defaults to ±2 semitones (±200 cents). Map cents to 14-bit bend value.
    int bend = (int)(8192.0 + (cents / 200.0) * 8192.0);
    if (bend < 0) bend = 0;
    if (bend > 16383) bend = 16383;
    for (int ch = 0; ch < 16; ++ch) {
        fluid_synth_pitch_bend(synths[sfId], ch, bend);
    }
}
